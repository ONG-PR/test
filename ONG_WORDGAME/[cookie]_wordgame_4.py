...
def check_user_word(user_word, prev_word, end):    # 자료에서 끝말잇기 조건에 부합하는 단어 검색
    for i in data:
        for j in data[i]:
            if user_word == j:    # 자료에 있을 때
                if prev_word == [''] or (end == user_word[:3]):    # 끝말 이어질 때  prev_word[-1][-3:]를 end로 바꾸기###########
...
def defind_end(word):
    ...
    if chosung[cho] == 'ㄴ' and jungsung[jung] in "ㅕㅛㅠㅣ":
        cho = 'ㅇ'

    elif chosung[cho] == 'ㄹ' and jungsung[jung] in "ㅑㅕㅖㅛㅠㅣ":
        cho = 'ㅇ'

    elif chosung[cho] == 'ㄹ' and jungsung[jung] in "ㅏㅐㅗㅚㅜㅡ":
        cho = 'ㄴ'

    else:
        pass

    #if cho == 2 and (jung in [6, 12, 17, 20]):
        #cho = 11

    #elif cho == 5 and (jung in [2, 6, 7, 12, 17, 20]):
        #cho = 11

    #elif cho == 5 and (jung in [0, 1, 8, 11, 13, 18]):
        #cho = 2


    char_num = chosung.index(cho) * 588 + jung * 28 + jong
    char_unicode = char_num + 0xAC00
    end_unicode = unichr(char_unicode)
    end = end_unicode.encode('utf8')
...
def find_next_word_for(data):
    ...
    while True:
        ...
        print("  prev_word=<%s>, user_word=<%s>, end=<%s>" % (prev_word[-1], user_word, user_word[-3:]))

        if check_user_word(user_word, prev_word, end):    # 끝말잇기 조건에 부합한 지 확인

            prev_word.append(user_word)    # 입력받은 단어가 끝말잇기 조건에 부합 & 중복되지 않으면 list에 저장
            end = defind_end(user_word)    # 입력받은 단어의 마지막 글자 저장
